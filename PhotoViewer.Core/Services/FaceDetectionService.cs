...
internal interface IFaceDetectionService
{
    ...
    Task<List<DetectedFaceModel>> DetectFacesAsync(SoftwareBitmap bitmapImage, CancellationToken cancellationToken = default);
}
...
internal class FaceDetectionService : IFaceDetectionService
{
    ...
    public async Task<List<DetectedFaceModel>> DetectFacesAsync(SoftwareBitmap softwareBitmap, CancellationToken cancellationToken = default)
    {
        if (!FaceDetector.IsSupported)
        {
            Log.Info("Could not detect faces: FaceDetector not supported on this device.");
